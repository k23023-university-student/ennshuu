...
public class HeroWorker extends Hero implements IHeroJob {

    public HeroWorker(String name, int hp, int atk, int def, int agi) {
...
//                + (new java.util.Random().nextInt(7) - 3); // ← −3〜+3のランダムな値
        ...
        //自身の攻撃力の9割を90%の確率で相手に与える
        int damage = (int)Math.ceil(this.getAtk() * 0.9);
        int successRate = 90;  // 攻撃成功率
